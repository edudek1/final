...
export default class App extends React.Component {
  ...
  getIdeas() {
    ...
    let airtableUrl = "https://api.airtable.com/v0/appnXLUy3rpJSjokd/shoplist?maxRecords=15&view=Grid%20view";

    // Needed for Airtable authorization, replace with your own API key
    let requestOptions = {
      headers: new Headers({
        'Authorization': 'Bearer keybjw4ou0JLwAsHk'
      })
    };
  ...
  upvoteIdea(data, secId, rowId, rowMap) {
    ...
    let airtableUrl = "https://api.airtable.com/v0/appnXLUy3rpJSjokd/shoplist/" + data.id;

    // Needed for Airtable authorization
    let requestOptions = {
      method: 'PATCH',
      headers: new Headers({
        'Authorization': 'Bearer keybjw4ou0JLwAsHk', // replace with your own API key
  ...
  else {rowMap[`${secId}${rowId}`].props.closeRow();
    }
}
  // Ignore an idea
  ...
  renderRow(data) {
    return (
      <ListItem style={{ paddingLeft: 20, paddingRight: 20 }}>
        ...
        <Right>
          <Text note>{data.fields.votes} Quantity</Text>
        </Right>
      </ListItem>
    )
  }
